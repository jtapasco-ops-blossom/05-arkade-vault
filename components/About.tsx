...
export function About() {
  ...
  const onSubmit = async (e: ReactSubmitEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(
          res.status === 429
            ? "[LÍMITE] DEMASIADOS MENSAJES — ESPERA UNOS MINUTOS"
            : "[ERROR] NO SE PUDO ENVIAR — INTENTA DE NUEVO"
        );
        setStatus("error");
      }
    } catch {
      setErrorMsg("[ERROR] NO SE PUDO ENVIAR — INTENTA DE NUEVO");
      setStatus("error");
    }
  };
            ...
            {status === "idle" || status === "sending" ? (
              ...
              <div className="terminal-success">
                ...
                <div className="term-body">
                  ...
                  <div style={{ marginTop: 18 }}>
                    <button className="btn ghost" type="button" onClick={reset}>ENVIAR OTRO MENSAJE</button>
                  </div>
                </div>
              </div>
            ) : (
              <div className="terminal-success terminal-error">
                ...
                <div className="term-body">
                  <div className="line"><span className="prompt">vault@arcade:~$</span> ./send_message --to=team</div>
                  <div className="line dim">[OK] Conectando con servidor…</div>
                  <div className="line success">&gt; {errorMsg}<span className="caret">_</span></div>
                  <div style={{ marginTop: 18 }}>
